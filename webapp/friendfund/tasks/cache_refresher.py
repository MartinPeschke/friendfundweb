...
def main(argv=None):
	...
	while 1:
		...
		cache_set_values = {FEATURED_POOLS_CACHEKEY: featured_pools, MERCHANTS_CACHEKEY:merchant_config.merchants, HOMEPAGE_STATS_CACHEKEY:merchant_config.stats}
		...
		log.info("CACHE_UPDATED (<%s>) (%s)", ">, <".join(map(lambda x: x.p_url, featured_pools)), ", ".join(["%s:%s"%(get_key_from_mfp_key(key), len(items)) for key, items in mfpools.iteritems()]))
